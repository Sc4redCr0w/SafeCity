...
export default function DashboardPage() {
  ...
  if (!data) {
    return (
      <div className="min-h-screen flex items-center justify-center text-gray-400 bg-slate-950">
        Loading dashboard…
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-950 text-white p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        <div className="rounded-xl bg-slate-900 p-4 border border-slate-800">
          ...
          <p className="text-2xl font-bold">{data.kpis.total_incidents}</p>
        </div>

        <div className="rounded-xl bg-slate-900 p-4 border border-slate-800">
          <p className="text-sm text-gray-400">High Risk Zones</p>
          <p className="text-2xl font-bold">{data.kpis.high_risk_zones}</p>
        </div>

        <div className="rounded-xl bg-slate-900 p-4 border border-slate-800">
          <p className="text-sm text-gray-400">Avg Confidence</p>
          <p className="text-2xl font-bold">
            {Math.round(data.kpis.avg_confidence * 100)}%
          </p>
        </div>
      </div>

      {/* INCIDENT TREND (TEXTUAL – STABLE) */}
      <div className="rounded-xl bg-slate-900 p-4 border border-slate-800 mb-8">
        <h2 className="font-medium mb-3">Incident Trend (Last 7 Days)</h2>
        <div className="grid grid-cols-7 gap-2 text-center text-sm">
          {data.trend.map((d: any, i: number) => (
            <div key={i} className="bg-slate-800 rounded p-2">
              ...
              <div className="font-semibold">{d.incidents}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* RISK DISTRIBUTION */}
      <div className="rounded-xl bg-slate-900 p-4 border border-slate-800 mb-8">
        <h2 className="font-medium mb-3">Risk Distribution</h2>
        ...
        <div className="space-y-2 text-sm">
          {data.heatmap_points
            ...
            .map((p: any, i: number) => (
              <div
                key={i}
                className="flex justify-between bg-slate-800 rounded p-2"
              >
                <span>{p.name}</span>
